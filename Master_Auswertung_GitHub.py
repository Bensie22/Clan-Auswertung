    ...
    <head>
        ...
        <style>
            ...
            @media (max-width: 768px) {{
                ...
                table:not(.radar-table):not(.war-hist-table), .wiki-table {{
                    width: 100%;
                    table-layout: auto;
                    border: none;
                ...
                table:not(.radar-table):not(.war-hist-table) th, .wiki-table th {{ display: none; }}
                table:not(.radar-table):not(.war-hist-table) tbody, table:not(.radar-table):not(.war-hist-table) tr, table:not(.radar-table):not(.war-hist-table) td,
                .wiki-table tbody, .wiki-table tr, .wiki-table td {{
                    display: block;
                    width: 100%;
                }}
                table:not(.radar-table):not(.war-hist-table) tr, .wiki-table tr {{
                ...
                table:not(.radar-table):not(.war-hist-table) td, .wiki-table td {{
                    border: none;
                    padding: 8px 0;
                    display: grid;
                ...
                table:not(.radar-table):not(.war-hist-table) td::before, .wiki-table td::before {{
                    color: #94a3b8;
                    font-weight: 700;
                    font-size: 0.86em;
                    text-transform: none;
                }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(1)::before, .wiki-table td:nth-child(1)::before {{ content: "Spieler"; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(2)::before, .wiki-table td:nth-child(2)::before {{ content: "Check"; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(3)::before, .wiki-table td:nth-child(3)::before {{ content: "Status"; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(4)::before, .wiki-table td:nth-child(4)::before {{ content: "Dabei"; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(5)::before, .wiki-table td:nth-child(5)::before {{ content: "Deck-Nutzung"; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(6)::before, .wiki-table td:nth-child(6)::before {{ content: "Ø Fame/Deck"; }}
                ...
                table:not(.radar-table):not(.war-hist-table) td:nth-child(7), .wiki-table td:nth-child(7),
                table:not(.radar-table):not(.war-hist-table) td:nth-child(9), .wiki-table td:nth-child(9) {{ white-space: nowrap; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(7) > *, .wiki-table td:nth-child(7) > *,
                table:not(.radar-table):not(.war-hist-table) td:nth-child(9) > *, .wiki-table td:nth-child(9) > * {{ white-space: nowrap; }}
                table:not(.radar-table):not(.war-hist-table) td:nth-child(9) .custom-tooltip.dotted,
        ...
        <div class="container">
            ...
            <div id="Wiki" class="tab-content">
                ...
                <div class="accordion-content">
                    ...
                    <ul>
                        <li><span class="i18n-de"><b>Die zweite Chance (Degradierung):</b> Wer als <i>Anführer</i>, <i>Vize</i> oder <i>Ältester</i> ohne Abmeldung nicht kämpft, wird nicht sofort entfernt, sondern genau <b>eine Rang-Stufe tiefer</b> gesetzt und bekommt so eine Bewährungschance.</span><span class="i18n-en"><b>The second chance (demotion):</b> Anyone with a <i>Leader</i>, <i>Co-leader</i> or <i>Elder</i> role who skips a war day without notice is not immediately removed, but is demoted exactly <b>one rank</b> as a probation chance.</span></li>
                        <li><span class="i18n-de"><b>Kein Spielraum für Mitglieder:</b> Wer als normales <i>Mitglied</i> (wie <b>Spieler B</b> oben) ohne Abmeldung gar nicht kämpft, verlässt den Clan. So bleibt Platz für verlässliche, aktive Spieler.</span><span class="i18n-en"><b>No leeway for Members:</b> Anyone with a regular <i>Member</i> role (like <b>Player B</b> above) who skips a war day without notice leaves the clan. This keeps room for reliable, active players.</span></li>
                        <li><span class="i18n-de"><b>Wieder ins Gleichgewicht kommen:</b> Wer nach einer Degradierung in der Folgewoche aktiv teilnimmt und alle Decks spielt, zeigt damit, dass er wieder dabei ist — und kann sich so seinen Rang zurückverdienen.</span><span class="i18n-en"><b>Getting back on track:</b> Anyone who, after a demotion, actively participates and plays all their decks the following week shows they are back on board — and can earn their rank back.</span></li>
                    </ul>
                </div>
...
.war-history-inner h4 {{ margin: 0 0 10px; color: #a78bfa; font-size: 0.88em; font-weight: 700; }}
.war-hist-table {{ width: 100%; border-collapse: collapse; font-size: 0.82em; max-width: 580px; }}
.war-hist-table th {{ text-align: center; color: #64748b; font-weight: 600; padding: 4px 10px; border-bottom: 1px solid rgba(255,255,255,0.08); font-size: 0.82em; text-transform: uppercase; }}
.war-hist-table td {{ text-align: center; padding: 5px 10px; border-bottom: 1px solid rgba(255,255,255,0.04); color: #e2e8f0; }}
.fame-bar-wrap {{ display: inline-block; width: 44px; height: 4px; background: rgba(255,255,255,0.1); border-radius: 2px; vertical-align: middle; margin-left: 5px; }}
.fame-bar {{ height: 100%; border-radius: 2px; background: #7c3aed; }}
.war-expand-icon {{ float: right; opacity: 0.35; font-size: 0.65em; margin-top: 4px; transition: transform 0.2s, opacity 0.2s; pointer-events: none; }}
...
(function(){{
  ...
  function buildTable(wars) {{
    if(!wars||!wars.length) return '<p class="war-loading"><span class="i18n-de">Keine Kriegsdaten vorhanden.</span><span class="i18n-en">No war data available.</span></p>';
    ...
    var rows=wars.slice(0,12).map(function(w){{
      ...
      return '<tr>'
        ...
        +'<td style="color:'+fameColor(f)+';font-weight:700;">'+f+' <span class="fame-bar-wrap"><span class="fame-bar" style="width:'+pct+'%"></span></span></td>'
    ...
    return '<table class="war-hist-table"><thead><tr>'
      +'<th><span class="i18n-de">Datum</span><span class="i18n-en">Date</span></th>'
      +'<th>Fame</th><th>Decks</th>'
      +'<th><span class="i18n-de">Boot</span><span class="i18n-en">Boat</span></th>'
      +'<th><span class="i18n-de">Platz</span><span class="i18n-en">Rank</span></th>'
