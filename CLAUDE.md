...
11. **Public shortcomings: one named exception, everything else stays private.** Donation flags (📦/💤) and warning badges (❌) are computed but must not be rendered next to player names in the public table — they belong in the leadership area. Reporting a clan-wide average is fine; putting a person on display is not. **The exception, decided by the clan lead on 22.09.2026:** unplayed decks are public — the 🎯 marker in the deck column plus the `🎯 Offene Decks` filter, driven by `data-offen` on each `<tr>`. Kept deliberately factual rather than accusatory (no ❌, no red, just a tooltip with the numbers): the point is to remind, not to shame. Players on registered vacation are exempt — `is_urlaub` clears `auffaellig`, because the rule book already takes them out of the clan values. Don't extend the exception to further metrics without the clan lead's say-so, and don't delete it as a "rule 11 violation" — it *is* the rule now.
